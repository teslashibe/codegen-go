...
// streamCommand implements streamingAgent for Codex. Codex `--json` emits
// JSONL whose objects use a different schema from Claude's stream-json, so
// the typed StreamEvent fields (Type, Result, ...) may be empty for Codex
// events; the original NDJSON line is always surfaced via [StreamEvent.Raw]
// so callers can decode Codex's schema themselves.
